...
async def get_all_resources(start_ind: int = 0, limit: int = 50, user_id: int = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def get_certain_resources(resource_type: str, start_ind: int = 0, limit: int = 50, user_id: int = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
